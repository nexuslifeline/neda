            ...
            <div class="static-content">
                <div class="page-content"><!-- #page-content -->
                    ...
                    <ol class="breadcrumb"  style="margin-bottom: 10px;">
                        ...
                        <li><a href="Purchases">Requisitions</a></li>
                    </ol>
                                ...
                                <div class="col-md-12">
                                    <div id="div_user_list">
                                        <div class="panel panel-default">
                                            ...
                                            <a data-toggle="collapse" data-parent="#accordionA" href="#collapseTwo"><div class="panel-heading" style="background: #2ecc71;border-bottom: 1px solid lightgrey;"><b style="color: white; font-size: 12pt;"><i class="fa fa-bars"></i> Requisitions</b></div></a>
                                    ...
                                    <div id="div_user_fields" style="display: none;">
                                        <div class="panel panel-default" style="border: 4px solid #2980b9;border-radius: 8px;">
                                            ...
                                            <div class="panel-body"  style="padding-bottom: 0%;padding-top: 0%;">
                                                ...
                                                <form id="frm_requistion" role="form" class="form-horizontal">
                                                    <h4 style="margin-bottom: 6px;"><b>REQ # : <span id="span_req_no">REQ-XXXX</span></b></h4>
                                                    <div style="border: 1px solid #a0a4a5;padding: 1%;border-radius: 5px;">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-sm-12">
                                                                Purpose * : <br />
                                                                <textarea name="purpose" class="form-control" placeholder="Purpose" data-error-msg="Purpose is required!" required></textarea>
                                                ...
                                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                    ...
                                                    <div class="row" style="display: none;">
                                                        <div class="col-lg-4 col-lg-offset-8">
                                                            <div class="table-responsive">
                                                                <table id="tbl_requisition_summary" class="table invoice-total" style="font-family: tahoma;">
                                                                    <tbody>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header " style="padding: 5px !important;">
                        <h2 style="color:white; padding-left: 10px;">Requisitions</h2>
                    </div>
                    <div class="modal-body">
...
<script>
    ...
    $(document).ready(function(){
        ...
        var initializeControls=function(){
            ...
            var createToolBarButton=function(){
                var _btnNew='<button class="btn btn-primary"  id="btn_new" style="text-transform: capitalize;font-family: Tahoma, Georgia, Serif;" data-toggle="modal" data-target="" data-placement="left" title="New Purcahase Order" >'+
                    '<i class="fa fa-plus"></i> New Requisitions</button>';
            ...
            /*var products = new Bloodhound({
             ...
             remote: {
             cache: false,
             url: 'Requisition/transaction/product-lookup/',
            ...
            _objTypeHead.typeahead(null, {
                ...
                $('#tbl_items > tbody').append(newRowItem({
                    ...
                    req_line_total_discount : "0.00",
                    tax_exempt : false,
                    req_tax_rate : tax_rate,
                    req_cost : purchase_cost,
                    req_discount : "0.00",
                    tax_type_id : null,
                    req_line_total : total,
                    req_non_tax_amount: net_vat,
                    req_tax_amount:vat_input
                }));
        ...
        var bindEventHandlers=(function(){
            ...
            $('#tbl_requisitions tbody').on( 'click', 'tr td.details-control', function () {
                ...
                if ( row.child.isShown() ) {
                    ...
                    /*$.ajax({
                     "dataType":"html",
                     "type":"POST",
                     "url":"Templates/layout/po/"+ d.requisition_id,
                     // "beforeSend" : function(){
            ...
            $('#tbl_requisitions tbody').on('click','button[name="edit_info"]',function(){
                ///alert("ddd");
                _txnMode="edit";
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.requisition_id;

                $('#span_req_no').html(data.requisition_no);
                ...
                $.ajax({
                    url : 'Requisition/transaction/items/'+data.requisition_id,
                    type : "GET",
                    cache : false,
                    dataType : 'json',
                    processData : false,
                    ...
                    success : function(response) {
                        ...
                        $.each(rows,function(i,value){
                            ...
                            $('#tbl_items > tbody').append(newRowItem({
                                ...
                                req_line_total_discount : value.req_line_total_discount,
                                tax_exempt : false,
                                req_tax_rate : value.req_tax_rate,
                                req_cost : value.req_cost,
                                req_discount : value.req_discount,
                                tax_type_id : null,
                                req_line_total : value.req_line_total,
                                req_non_tax_amount: value.non_tax_amount,
                                req_tax_amount:value.tax_amount
                            }));
                        });
            ...
            $('#tbl_requisitions tbody').on('click','button[name="remove_info"]',function(){
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.requisition_id;
            ...
            $('#tbl_items tbody').on('keyup','input.numeric,input.number',function(){
                ...
                var price=parseFloat(accounting.unformat(row.find(oTableItems.unit_cost).find('input.numeric').val()));
                ...
                var discounted_cost=price-discount;
                var line_total_discount=discount*qty;
                var line_total=discounted_cost*qty;
            ...
            $('#btn_yes').click(function(){
                ...
                //showNotification({title:"Error!",stat:"error",msg:"Sorry, you cannot delete Requisitions that is already been recorded on purchase invoice."});
                //}else{
                removeRequisition().done(function(response){
                    showNotification(response);
            ...
            $('#btn_save').click(function(){
                ...
                if(validateRequiredFields($('#frm_requistion'))){
                    if(_txnMode=="new"){
                        createRequisition().done(function(response){
                            showNotification(response);
        ...
        var createRequisition=function(){
            var _data=$('#frm_requistion,#frm_items').serializeArray();

            var tbl_summary=$('#tbl_requisition_summary');
            ...
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Requisition/transaction/create",
                "data":_data,
                "beforeSend": showSpinningProgress($('#btn_save'))
            });
        };

        var updateRequistion=function(){
            var _data=$('#frm_requistion,#frm_items').serializeArray();
            ...
            _data.push({name : "summary_tax_amount", value : tbl_summary.find(oTableDetails.tax_amount).text()});
            _data.push({name : "summary_after_tax", value : tbl_summary.find(oTableDetails.after_tax).text()});
            _data.push({name : "requisition_id" ,value : _selectedID});
            _data.push({name : "remarks", value : $('textarea[name="remarks"]').val() });

            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Requisition/transaction/update",
                "data":_data,
                "beforeSend": showSpinningProgress($('#btn_save'))
            });
        };

        var removeRequisition=function(){
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Requisition/transaction/delete",
                "data":{requisition_id : _selectedID}
            });
        };
